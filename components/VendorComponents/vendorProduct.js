...
function VendorProduct({ product, refetch }) {
  ...
  async function savePrice() {
    ...
    try {
      if (product.combination) {
        const response = await Api.put(`/api/vendor/edit-price/${product.id}`, {
    ...
    try {
      if (product.combination) {
        const response = await Api.delete(
          `/api/vendor/delete-product/${product.id}`,
          {
            headers: {},
    ...
    <>
      ...
      >
        <DialogTitle className=" border-b-2 border-gray-200 flex justify-between items-center">
          ...
          <MdClose
            className="w-[25px] h-[25px] text-red-500 hover:text-red-600 cursor-pointer "
            onClick={() => {
              setIsAddingCobination(false);
              setCombinations();
            }}
