...
function AdminSearchDropDown({ data, title, selectItem }) {
  ...
  function onSelectItem(item) {
    selectItem(item);
    setSelectedItem(item.name_en ?? item.name);
    setIsOpen(false);
  }
  ...
  return (
    <div
      className=" relative md:w-[400px] w-full border-b-2 border-gray-300"
      ref={dropdownRef}
    >
      <div
        onClick={() => {
      ...
      {isOpen == true && (
        <div className="absolute -top-100 right-0 left-0 bg-white px-2 py-1 border rounded-lg border-skin-primary ">
