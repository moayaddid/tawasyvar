...
import SellerSelecteableProduct from "@/components/SellerComponents/SellerSelecteableProduct";
...
function ProductsManagment() {
  ...
  const {
    ...
    ["ProductsManagmentAllProducts", brandFilters, categoryFilters],
  ...
  async function fetchCategoriesBrands() {
    ...
    try {
      const allCategories = await Api.get(`/api/seller/categories-store`);
      setCategories(allCategories?.data?.data);
      const allBrands = await Api.get(`/api/seller/brands-store`);
  ...
  async function applyPrice() {
    ...
    if (allProductsSelected && allProductsSelected == true) {
      try {
        const response = await Api.post(`/api/seller/update-all-prices`, {
      ...
      try {
        ...
        selectedProducts.map((product) => {
          pros.push({ store_product_id: product.id });
        });
        const response = await Api.post(`/api/seller/edit-price-batch`, {
          is_increase: Number(increase) === 1 ? true : false,
          amount: amount,
          store_products: pros,
        });
  ...
  async function applyAvailability() {
    ...
    if (allProductsSelected && allProductsSelected == true) {
      try {
        const response = await Api.post(`/api/seller/update-all-availability`, {
          availability: status,
        });
      ...
      try {
        ...
        const response = await Api.post(`/api/seller/edit-availability-batch`, {
          availability: status,
          store_products: pros,
        });
        refetch();
  ...
  async function search(e) {
    ...
    try {
      const response = await Api.get(
        `api/seller/search-all-products/${storeId}`,
        {
...
export default withLayout(ProductsManagment);
