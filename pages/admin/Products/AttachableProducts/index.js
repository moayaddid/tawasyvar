...
function AttachableProducts() {
  ...
  async function fectAttachableProducts(selectedStore, selectedPage) {
    // console.log(`in fetching function`)
    try {
      if (selectedStore && selectedPage) {
        return await Api.get(
          `/api/admin/get-waffer-products/${selectedStore}?page=${selectedPage}`
        );
  ...
  // useEffect(() => {
  //   // console.log(attachableProducts);
  // }, [attachableProducts, selectedStore, currentPage]);
  ...
  return (
    <div className="w-[90%] mx-auto py-12 flex flex-col space-y-4 h-screen ">
      ...
      <div className="flex justify-start space-x-6 items-center">
        ...
        {attachableProducts && attachableProducts?.data?.pagination && <p>-</p>}
      ...
      {isLoading == true ? (
        ...
        <div className="w-full">
          ...
          attachableProducts.data?.products?.length > 0 ? (
                  ...
                  attachableProducts.data.products.map((prod) => {
                    return (
                      <AdminAttachableProduct
                        product={prod}
                        AttachedStore = {attachableProducts.data.store}
            ...
            <p className="text-center text-lg">There are no Attachable Products</p>
          )}
        </div>
      )}
    </div>
