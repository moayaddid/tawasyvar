...
function AcceptedOrdersAdmin() {
  ...
  const {
    ...
    refetch,
    isRefetching,
  } = useQuery("adminAcceptedOrders", fetchAcceptedOrders, {
    staleTime: 1,
  ...
  async function fetchAcceptedOrders () {
    try{
      return await Api.get(`/api/admin/accepted-orders`);
    }catch(error){
    ...
    <div className="md:px-6">
      <div className="container">
        <div className="m-5 p-5">
          <h2 className="text-2xl text-stone-500 pb-5 ">Accepted Orders</h2>
        </div>
        ...
        <div className="w-full ">
          ...
          </table> : <div> There are no accepted orders. </div>}
        </div>
      </div>
    </div>
  );
...
export default withLayoutAdmin(AcceptedOrdersAdmin);
