...
function PendingOrdersAdmin() {
  ...
  const {
    ...
    refetch,
    isRefetching,
  } = useQuery("adminPendingOrders", fetchPendingOrders, {
    staleTime: 1,
  ...
  async function fetchPendingOrders () {
    try{
      return await Api.get(`/api/admin/pending-orders`);
    }catch(error){
    ...
    <div className="md:px-6">
      <div className="container">
        <div className="m-5 p-5">
          <h2 className="text-2xl text-stone-500 pb-5 ">Pending Orders</h2>
        </div>
        ...
        <div className="w-full ">
          ...
          </table> : <div> There are no pending orders. </div>}
        </div>
      </div>
    </div>
  );
...
export default withLayoutAdmin(PendingOrdersAdmin);
